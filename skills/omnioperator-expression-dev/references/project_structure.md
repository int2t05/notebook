...
>
> ⚠️ **注册名 = function_name(大小写敏感)**:`RegisterFunction(prefix + "name", ...)` 的注册名必须与 OmniAdaptor `RexNodeUtil` 生成的 `function_name` 完全一致,否则 native 报 `Function not supported`。反例 `char_length`:注册名 `"length"` ≠ SQL `char_length`。
...
// Date/Time
OMNI_DATE32        // date
OMNI_TIMESTAMP     // timestamp

// Complex types
OMNI_ARRAY         // ArrayVector
OMNI_MAP           // MapVector
OMNI_ROW           // RowVector
OMNI_CONTAINER     // container
```
