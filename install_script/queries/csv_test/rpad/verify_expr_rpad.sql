-- =============================================================================
-- RPAD 向量化表达式 — CSV 黄金对比 SQL
-- 语义: flink/docs/data/sql_functions.yml —
--       RPAD(string1, integer, string2):
--       用 string2 在右侧填充 string1 到 integer 字符长;
--       若 string1 长于 integer, 则把 string1 截断到 integer 字符长。
--       例: RPAD('hi', 4, '??') -> "hi??"; RPAD('hi', 1, '??') -> "h"。
--       yaml 未文档化 NULL/负数/0 边界, 以下期望以 OmniOperator 向量化实现为准。
--
-- !! 实现说明 (重要) !!
--   OmniOperator 已有向量化实现 (functions/String.h, PadFunctionBase<false,T> =
--   RPadFunction), 注册名 "rpad" (registration/RegisterString.cpp), 8 个重载:
...
--     * 原串字符数 <  size  -> 右侧用 padString 循环填充到 size 字符
...
--
-- 上传 CSV 到服务器:
--   install_script/queries/csv_test/rpad/verify_expr_rpad.csv
--   -> /opt/buildtools/install_script/queries/csv_test/rpad/verify_expr_rpad.csv
--
...
--   $FLINK_HOME/bin/sql-client.sh -f /tmp/verify_expr_rpad.sql
--   # 确认走 native:
...
CREATE TABLE src (
  ...
  'path' = '/opt/buildtools/install_script/queries/csv_test/rpad/verify_expr_rpad.csv',
  'format' = 'csv',
...
-- r05    | ab     | 5   | xyz | abxyz       (多字符 pad, 不完整取前缀)
-- r06    | ab     | 5   | #   | ab###       (单字符 pad 循环)
-- r07    | abc    | 3   | ?   | abc         (原串长度恰等于 len, 不填充)
-- r08    | 你好   | 4   | ab  | 你好ab      (Unicode 按字符计数, 非 4 字节)
-- r09    | (空串) | 3   | ?   | ???         (空原串, 全填充)
