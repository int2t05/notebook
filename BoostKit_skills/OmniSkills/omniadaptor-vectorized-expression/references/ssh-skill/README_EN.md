...
> Enterprise-grade SSH management tool for Codex / zcode Code, making remote server operations as simple and efficient as local ones
...
- Multiple zcode Code instances share connections
...
2. For zcode Code: place the `ssh-skill` directory under `.zcode/skills/ssh-skill/`.
...
```bash
python .zcode/skills/ssh-skill/scripts/ssh_execute.py prod-web-01 "systemctl status nginx"
```
...
```bash
# Small files (fast)
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_upload.py prod-web-01 ./app.tar.gz /tmp/

# Large files (auto progress display)
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_upload.py prod-web-01 ./large-file.iso /tmp/

# Resume support
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_upload.py prod-web-01 ./large-file.iso /tmp/ --resume

# Recursive directory upload
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_upload.py prod-web-01 ./dist/ /var/www/html/ --recursive
```

### Download Files

```bash
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_download.py prod-web-01 /var/log/app.log ./app.log
```

### Server-to-Server Transfer

```bash
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_server_transfer.py source-server /data/backup.tar.gz target-server /backup/
```
...
## 🎨 Integration with Codex / zcode Code

In Codex or zcode Code, AI uses the `SKILL.md` description to automatically select ssh-skill for SSH operations:
