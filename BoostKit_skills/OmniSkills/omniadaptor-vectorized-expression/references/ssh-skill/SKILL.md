...
1. 运行命令获取数据：
```bash
python .zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py list-servers
```
2. 解析返回的 JSON 数据
...
- 自动连接复用：多个 zcode Code 实例可共享同一守护进程
...
**默认路径**：`.zcode/skills/ssh-skill/scripts`
- `~` 会自动展开为用户家目录（Windows 和 Linux 通用）
- Windows: `C:\Users\用户名\.zcode\skills\ssh-skill\scripts`
- Linux: `/home/用户名/.zcode/skills/ssh-skill/scripts`

**项目目录中的 skill**：如果 skill 放在项目的 `.zcode/skills/ssh-skill/` 中，使用相对路径：
```
.zcode/skills/ssh-skill/scripts
```
...
**MUST**: 使用 `python .zcode/skills/ssh-skill/scripts/脚本名.py` 格式。使用别名（alias）标识服务器。
...
```bash
python .zcode/skills/ssh-skill/scripts/ssh_execute.py <别名> "<命令>"
```
...
```bash
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_upload.py <别名> "<本地路径>" "<远程路径>"
```
...
```bash
MSYS_NO_PATHCONV=1 python .zcode/skills/ssh-skill/scripts/ssh_download.py <别名> "<远程路径>" "<本地路径>"
```
...
MSYS_NO_PATHCONV=1 python ".zcode/skills/ssh-skill/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>"

# 强制直连模式（大文件推荐，数据直接在服务器间传输）
MSYS_NO_PATHCONV=1 python ".zcode/skills/ssh-skill/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --mode direct

# 强制流式转发（小文件或服务器间网络不通时）
MSYS_NO_PATHCONV=1 python ".zcode/skills/ssh-skill/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --mode stream

# 混合模式（先尝试直连，失败后自动降级到流式）
MSYS_NO_PATHCONV=1 python ".zcode/skills/ssh-skill/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --mode hybrid

# 使用 rsync（仅直连模式，支持增量同步）
MSYS_NO_PATHCONV=1 python ".zcode/skills/ssh-skill/scripts/ssh_server_transfer.py" <源别名> "<源路径>" <目标别名> "<目标路径>" --use-rsync
```
...
```bash
# 对所有服务器执行
python ".zcode/skills/ssh-skill/scripts/ssh_cluster.py" "<命令>" --parallel

# 对指定别名列表执行
python ".zcode/skills/ssh-skill/scripts/ssh_cluster.py" "<命令>" --hosts "DEV-002,DEV-003" --parallel

# 按环境过滤
python ".zcode/skills/ssh-skill/scripts/ssh_cluster.py" "<命令>" --environment production --parallel

# 按标签过滤
python ".zcode/skills/ssh-skill/scripts/ssh_cluster.py" "<命令>" --tags "web,nginx" --parallel
```
...
# 按环境过滤
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" list-servers --environment production

# 查找服务器（支持别名和描述模糊查找）
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" find "<关键词>"

# 创建配置
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" create --alias <别名> --host <IP> --user <用户名> --key <密钥文件> --environment <环境>

# 更新配置（只更新提供的字段，其他字段保持不变）
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" update <别名> --description "新描述" --tags tag1 tag2 tag3
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" update <别名> --environment production --location "新位置"
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" update <别名> --host <新IP> --port <新端口>

# 删除配置
python ".zcode/skills/ssh-skill/scripts/ssh_config_manager_v3.py" delete <别名>
```
...
```bash
# 启动 tunnel（自动分配本地端口）
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" start <别名> --remote-port <端口>

# 指定本地端口
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" start <别名> --local-port <本地端口> --remote-port <远程端口>

# 转发到远程的其他主机
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" start <别名> --remote-host <远程主机> --remote-port <端口>

# 列出所有活动的 tunnel
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" list

# 查看 tunnel 状态
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" status <tunnel-id>

# 停止 tunnel
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" stop <tunnel-id>

# 停止服务器的所有 tunnel
python ".zcode/skills/ssh-skill/scripts/ssh_tunnel.py" stop-all <别名>
```
...
```bash
# 启动守护进程（通常不需要手动启动）
python ".zcode/skills/ssh-skill/scripts/ssh_daemon.py" start <别名>

# 查看守护进程状态
python ".zcode/skills/ssh-skill/scripts/ssh_daemon.py" status <别名>

# 停止守护进程
python ".zcode/skills/ssh-skill/scripts/ssh_daemon.py" stop <别名>
```
...
- 每台服务器独立守护进程，按别名隔离
- 多个对话（多个 zcode Code 实例）可共享同一守护进程
- SSH 连接断开自动重连（最多 3 次）
- 每 60 秒心跳检测连接状态
- 空闲超时自动退出，无需手动清理
...
```bash
python ".zcode/skills/ssh-skill/scripts/ssh_execute.py" <别名> "<命令>" --no-daemon
```
